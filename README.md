...
```Viridula_txdb_parsed <- makeTxDbFromGFF(file="{PWD}/Scurria_viridula_annotation_v1_ch10_top10.gtf", format="gtf")```
...
```txdf <- AnnotationDbi::select(S.viridula.p,keys=keys(S.viridula.p, "GENEID"),columns=c("GENEID", "TXID", "TXCHROM","TXNAME","EXONID", "EXONNAME"),keytype="GENEID")```
...
```mcols(exons.list.per.transcript)```
...
```Zebrina_txdb_parsed <- makeTxDbFromGFF(file="{PWD}/Scurria_zebrina_annotation_v1_ch10_top10.gtf", format="gtf")```
...
```txdf <- AnnotationDbi::select(S.zebrina.p,keys=keys(S.zebrina.p, "GENEID"),columns=c("GENEID", "TXID", "TXCHROM","TXNAME","EXONID", "EXONNAME"),keytype="GENEID")```
...
Add column with Orthologous chromosome distinction based on MCSCAN collinearity plot (orthologous chromosomes in these three species do not have the same number)
...
```boxplot(num_exons ~ ORTHGRP, data = merged_df)```

Kruskal Wallis tests

```kruskal.test(num_exons ~ spID, data = merged_df)```

```pairwise.wilcox.test(merged_df$num_exons, merged_df$spID, p.adjust.method = "fdr")```
